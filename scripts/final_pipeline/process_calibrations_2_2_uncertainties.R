...
all_cal_times <- read_csv("/mnt/scratch/users/cw1781/btt_cal_processing/btt_nox_processing/data/data/cal_times_all.csv") |> 
...
cal_file <- all_cal_times |>
  arrange(date)  
  filter(date >= as.POSIXct("2022-08-01", tz = "UTC"),
         date <  as.POSIXct("2022-09-01", tz = "UTC"))

# List all Parquet parameter files
param_root <- "/mnt/scratch/projects/chem-cmde-2019/btt_processing/processing/raw_parquet/data/params_edit/"
...
for (i in 1:length(cal_times)) {
  ...
  zero_data_median <- zero_data |> 
    ...
    summarise(zero_ch1 = median(ch1_hz, na.rm = TRUE), 
              ...
              zero_ch1_SD = sd(ch1_hz, na.rm = TRUE), 
              zero_ch2_SD = sd(ch2_hz, na.rm = TRUE))
  ...
  if (all(required_multi %in% flows_present)) {
    ...
    #adding in the uncertainties 
    fit_ch1 <- lm(ch1_hz ~ cal_conc, data = cal_data)
    fit_ch2 <- lm(ch2_hz ~ cal_conc, data = cal_data)
    
    se_ch1 <- summary(fit_ch1)$coefficients[2,2]
    se_ch2 <- summary(fit_ch2)$coefficients[2,2]
    
    ureg_ch1 <- se_ch1 / cal_slope_ch1
    ureg_ch2 <- se_ch2 / cal_slope_ch2
    
    cal_timestamp2 <- format(cal_timestamp, "%Y-%m-%d %H:%M:%S") 
    ...
    cal_data <- cal_data %>%
      mutate(
        ...
        ureg_ch1 = ureg_ch1,
        ureg_ch2 = ureg_ch2,
        date = as.POSIXct(cal_timestamp2, format = "%Y-%m-%d %H:%M:%S", tz ="UTC")
      )
  ...
  all_stages <- bind_rows(gpt_0_blc_1,
    ...
    summarise(
      ...
      zero_ch1_sd = zero_data_median$zero_ch1_sd, 
      zero_ch2_sd = zero_data_median$zero_ch2_sd,
      .groups = "drop") |> 
  ...
  con_eff2 <- bind_rows(gpt_0_blc_1,
    ...
    summarise(
      ...
      ch1_hz_mean = mean(ch1_hz, na.rm = TRUE),
      ch2_hz_mean = mean(ch2_hz, na.rm = TRUE),
      ch1_hz_sd = sd(ch1_hz, na.rm = TRUE),
      ch2_hz_sd = sd(ch2_hz, na.rm = TRUE),
      ch1_SE = ch1_hz_sd / sqrt(n()),
      ch2_SE = ch2_hz_sd / sqrt(n()),
...
cal_coefficients |>
  ...
  group_walk(~ {
    output_dir <- file.path("/mnt/scratch/projects/chem-cmde-2019/btt_processing/processing/calibration_data/test", .y$year, .y$month_num)
