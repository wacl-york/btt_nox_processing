...
outputFile = "/mnt/scratch/users/cw1781/btt_cal_processing/btt_nox_processing/logs/make_ec_inputs_2022/%x_%j_%a.log" #change to hardcoded path
errFile = "/mnt/scratch/users/cw1781/btt_cal_processing/btt_nox_processing/logs/make_ec_inputs_2022/%x_%j_%a.err"
...
files = tibble(
  ...
  mutate(fileDate = as.POSIXct(fileName, format = "NOx_5Hz_%y_%m_%d_%H%M%S.csv", tz = "UTC"),
         idx = row_number()-1) %>% 
  filter(fileDate  >= ymd_hms("2022-01-01 00:00:00"))
...
message = c("#!/usr/bin/env bash",
            ...
            "",
            "# Commands to run",
            paste0("apptainer exec --bind /mnt/scratch/projects/chem-cmde-2019/btt_processing/:/data/,/mnt/scratch/users/cw1781/btt_cal_processing/btt_nox_processing/scripts/:/scripts/ /mnt/longship/projects/chem-cmde-2019/eddy4r/eddy4r.york_dev Rscript /scripts/get_flux_input_2022.R $SLURM_ARRAY_TASK_ID")
)

data_file = file(paste0('/mnt/scratch/users/cw1781/btt_cal_processing/btt_nox_processing/sbatch/get_inputs_2022.sbatch'), open = "wt")
