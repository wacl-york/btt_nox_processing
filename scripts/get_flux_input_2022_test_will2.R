...
out_root <- "/data/processing/ec/in_test/will2"
...
df_5hz_final <- as_tibble(dt_5hz) %>%
  ...
  mutate( # original 
    no_a  = (ch1_hz - ch1_zero) / ch1_sens,
    noc_a = (((ch2_hz - ch2_zero) / ch2_sens) - no_a)
  ...
  mutate( # pretty sure this is right.
    no_c =  ((ch1_hz - ch1_zero) / ch1_sens),
    noc_c = (((ch2_hz - ch2_zero) / ch2_sens)) - no_c
  ) 
